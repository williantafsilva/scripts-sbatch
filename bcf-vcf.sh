...
##find <PATH TO DIRECTORY>/*.bcf -maxdepth 0 | while read F ; do 
##    sbatch \
##			-A ${PROJECT_ID} \
##			-o ${MYSLURMFILE} \
##			-p shared \
...
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.bcftovcf-job${JOBID}.vcf.gz") 
...
bcftools convert -O z -o ${OUTPUTFILE} ${INPUTFILE}
