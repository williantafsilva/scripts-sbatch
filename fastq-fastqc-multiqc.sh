...
##Input $2:
##Input $3:
##Input $4: VCF file (.vcf or .vcf.gz).
...
##			-n 5 \ ##--mem=100GB \
##			-t 0-12:00:00 \
##			-J fastq-fastqc-multiqc-${F##*/} \
##			--dependency=afterok:<JOB1 ID>:<JOB2 ID> \
##			fastq-fastqc-multiqc.sh <OUTPUT LOCATION> ${F} 
##done
...
SCRIPTNAME=$(echo "fastq-fastqc-multiqc.sh") 
...
#!bin/bash
#$1 is the path to the folder containing the individual samples folder.
#$2 is the path to the folder containing the output. It must exist
#$3 is the path  of the multiqc output folder
#example: bash code/00_qc.sh 01_trimming/Feb2020/L004/paired/ 02_quality_check/Feb2020/L004/ 02_quality_check/Feb2020/L004/multiqc/

for file in $1/*_R1_fastp.fastq.gz
do
#id=${file##*/}
name=${file%_R*}
fastqc -o $2 \
-t 16 \
$name'_R1_fastp.fastq.gz' $name'_R2_fastp.fastq.gz'
done
