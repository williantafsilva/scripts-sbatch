...
##			template-sbatchscript.sh <OUTPUT LOCATION> ${F} 
##done
...
SCRIPTNAME=$(echo "template-sbatchscript.sh") ##?????
...
echo 
echo "############################################################################"
echo "##PROCESSING FILE/DIRECTORY: ${INPUTFILE}" ##?????
echo 
############################################################################
############################################################################
############################################################################
...
##cat $0 > $(echo "${OUTPUTDIR}/job${JOBID}.script") ##When the output is a directory.
cat $0 > ${SUBMITTEDSCRIPT}
cat ${SLURMFILE} > $(echo "${OUTPUTLOCATION}/job${JOBID}.slurm") 
##cat ${SLURMFILE} > $(echo "${OUTPUTDIR}/job${JOBID}.slurm") ##When the output is a directory.
