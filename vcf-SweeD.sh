...
##Input $2: VCF file (.vcf.gz) or SweepFinder2 input file (.SFinput), split by chromosome.
...
##Run SweeD.
SweeD -name ${OUTPUTFILESUFFIX} -input ${INPUTFILEX} -grid ${NSITES}   
sleep 5s
...
if [[ ${INPUTFILE} == *.vcf.gz ]] ; then ##If VCF file format.
	rm -f ${INPUTFILEX} ##Delete .vcf file.
fi
