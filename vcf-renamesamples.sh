...
##			-n 5 \ ##--mem=100GB \
##			-t 0-12:00:00 \
##			-J vcf-renamesamples-${F##*/} \
##			--dependency=afterok:<JOB1 ID>:<JOB2 ID> \
##			vcf-renamesamples.sh <OUTPUT LOCATION> ${F} <SAMPLE NAME CONVERTION FILE>
##done
...
SAMPLENAMECONVERTIONFILE: ${SAMPLENAMECONVERTIONFILE}
"
...
OUTPUTFILENAME=$(echo "${OUTPUTFILEPREFIX}.renamesamples-job${JOBID}.vcf.gz") 
...
bcftools reheader --samples ${SAMPLENAMECONVERTIONFILE} --output ${OUTPUTFILE} ${INPUTFILE}
